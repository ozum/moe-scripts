...
test('resolveKcdScripts resolves to src/index.js when in the moe-scripts package', () => {
  mockPkg({pkg: {name: 'moe-scripts'}})
  expect(require('../utils').resolveKcdScripts()).toBe(
    require.resolve('../').replace(process.cwd(), '.'),
  )
})

test('resolveKcdScripts resolves to moe-scripts if not in the moe-scripts package', () => {
  mockPkg({pkg: {name: 'not-moe-scripts'}})
  whichSyncMock.mockImplementationOnce(() => require.resolve('../'))
  expect(require('../utils').resolveKcdScripts()).toBe('moe-scripts')
})
